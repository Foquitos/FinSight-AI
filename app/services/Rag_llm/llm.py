...
class ChatBot:
    ...
    @staticmethod
    def _format_citations(source_nodes_data: List[Dict]) -> str:
        """
        Builds a deterministic 'Sources' footer from the retrieved nodes.

        Citations are appended programmatically (not generated by the LLM) so
        every grounded answer reliably cites the financial documents it used,
        as required for an auditable analyst tool.
        """
    ...
    def query(self, query_text: str, user_id: int, task_id: str) -> QueryResponse:
        ...
        try:
            ...
            # Append a deterministic citation footer so the answer always
            # cites the financial documents it was grounded in.
            response_text = f"{response_text}{self._format_citations(source_nodes_data)}"
    ...
    async def stream_query(self, query_text: str, user_id: int, task_id: str):
        ...
        try:
            ...
            # Stream a deterministic citation footer so the answer always
            # cites the financial documents it was grounded in.
            citations = self._format_citations(source_nodes_data)
            if citations:
                yield citations
            full_response = f"{full_response}{citations}"
